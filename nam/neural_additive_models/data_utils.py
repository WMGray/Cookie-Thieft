...
def read_dataset(dataset_name,DATA_PATH1,header = 'infer',names = None,delim_whitespace = False):
  dataset_path = osp.join(DATA_PATH1, dataset_name)
...
def load_recidivism_data(DATA_PATH1):
  ...
  # Create column names
  attr_dict_path = osp.join(DATA_PATH1, 'recidivism/data_recidivism_recid.attr')
  attributes = gfile.Open(attr_dict_path, 'r').readlines()
  column_names = [x.split(':')[0] for x in attributes]

  df = read_dataset(
      'recidivism/data_recidivism_recid.data',
      DATA_PATH1,
      header=None,
      names=column_names,
      delim_whitespace=True)
...
def load_dataset(dataset_name):
  ...
  if dataset_name == 'Telco':
    ...
    dataset = load_recidivism_data('/hde/gwc_data/')
